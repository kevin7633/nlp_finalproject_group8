...
def paraphrase_prompt(q1, q2, template_id=1):
  if template_id == 1:
    return f'Is "{q2}" a paraphrase of "{q1}"? Answer yes or no: '
  if template_id == 2:
    return f'Question 1: {q1}\nQuestion 2: {q2}\nAre these two questions asking the same thing? Answer yes or no: '
...
class ParaphraseDetectionDataset(Dataset):
  ...
  def collate_fn(self, all_data):
    ...
    cloze_style_sents = [
      paraphrase_prompt(s1, s2, getattr(self.p, 'template_id', 1))
    ...
    encoding = self.tokenizer(
      ...
      padding=True,
      truncation=True,
      max_length=getattr(self.p, 'max_length', 128)
    )
